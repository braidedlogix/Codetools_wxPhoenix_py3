...
def test_namespace_decorator():
    ...
    # iteration:
    assert_equal(
        sorted(item for item in results
               if item[0][0] != "_"), [('x', 1), ('y', 2)])
...
def test_namespace_in_execfile():
    ...
    exec(compile(open(tempfile).read(), tempfile, 'exec'), dic)
    assert_equal(
        set(dic.keys()), set(['a', '__builtins__', 'namespace', 'f', 'ns']))
    ns = dic['ns']
    assert_equal(ns.x, 1)
    assert_equal(ns.y, 2)
    os.remove(tempfile)
