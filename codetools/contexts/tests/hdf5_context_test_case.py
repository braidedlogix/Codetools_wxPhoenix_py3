...
class Particle(tables.IsDescription):
    identity = tables.StringCol(
        itemsize=22, dflt=" ", pos=0)  # character String
...
def create_sample_hdf5_file(filename):
    ...
    array1 = fileh.createArray(root, "array1", ["string", "array"],
                               "String array")
...
class Hdf5ContextTest(unittest.TestCase):
    ...
    def test_empty_keys1(self):
        ...
        print('keys (should be empty):', list(context.keys()))
        print()
        table.close()
    ...
    def test_empty_keys2(self):
        ...
        table = tables.openFile('test.h5')
        context = Hdf5Context(
            file_object=table, path=['root', 'root.group1', 'root.group2'])
        print('keys (should have a lot of names):', list(context.keys()))
    ...
    def test_use_as_context(self):
        ...
        table = tables.openFile('test.h5')
        hdf_context = Hdf5Context(
            file_object=table, path=['root', 'root.group1', 'root.group2'])
        results = DataContext()
        context = MultiContext(results, hdf_context)
        exec('array2_plus_one = [a2 + 1 for a2 in array2]', {}, context)
